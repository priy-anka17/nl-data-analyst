...
2. Store your final answer in a variable called `result`. It should be a string, number, or DataFrame.
3. If a chart would help, create a Plotly figure and store it in a variable called `fig`. Use plotly.express or plotly.graph_objects.
...
10. For groupby/aggregations, reset_index() so result is a clean DataFrame.
"""
...
def _execute_code(code: str, df: pd.DataFrame) -> tuple[str, dict | None, str]:
    ...
    if result is None:
        ...
        output = result.to_string(max_rows=50, max_cols=20)
